...
window.updateGameSettings = function () {
  ...
  gameSettings.glowEffects = settings.glowEffects;
  ...
  collisionSound.volume = gameSettings.soundEffects / 100;
  explosionSound.volume = gameSettings.soundEffects / 100;
  updateAllGlowEffects();
};
function applyGlowEffect(color) {
  const glowIntensity = gameSettings.glowEffects / 100;
  if (glowIntensity > 0) {
    return {
      fillStyle: color,
      strokeStyle: "#FFFFFF",
      lineWidth: 2 + Math.floor(glowIntensity * 3),
      shadowColor: color,
      shadowBlur: 15 + Math.floor(glowIntensity * 25),
...
function updateAllGlowEffects() {
  pieces.forEach((piece) => {
    if (piece.render) {
      const glowStyle = applyGlowEffect(piece.color);
      Object.assign(piece.render, glowStyle);
    }
  });
}
